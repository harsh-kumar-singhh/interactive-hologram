...
const monumentAudio = {
  ...
  trex: new Audio("roar/trex-roar.mp3"),
  triceratops: new Audio("roar/Triceratops.mp3"),
  brachiosaurus: new Audio("roar/Brachiosaurus.mp3"),
  stegosaurus: new Audio("roar/Stegosaurus.mp3"),
  velociraptor: new Audio("roar/Velociraptor.mp3"),
  spinosaurus: new Audio("roar/Spinosaurus.mp3"),
};
...
const MODEL_CONFIGS = {
  eiffel: {
    zoomMinFactor: 1.0,
    zoomMaxFactor: 4.0,
    customTargetHeight: 5.5,
    isEiffel: true,
  },
  bigben: {
    zoomMinFactor: 1.0,
    zoomMaxFactor: 3.5,
    customLightIntensity: 6.0,
    minRotationX: -0.3,
    maxRotationX: 0.3,
...
function resolveModelPath(modelName) {
  ...
  if (dinosaurModels.includes(modelName)) {
    return `./dinosaurs/${modelName}.glb`;
  }

  if (modelName.toLowerCase() === "burj_khalifa") {
...
async function loadModel(modelName) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      let scaleFactor =
        (config?.customTargetHeight || 5.0) / (box.max.y - box.min.y);
      ...
      targetTransform.scale = currentTransform.scale =
        scaleFactor * startingZoomFactor;
      ...
      transformVelocity.rotationX =
        transformVelocity.rotationY =
        transformVelocity.scale =
          0;
    ...
    (err) => {
      console.error(
        "Failed to parse resource target asset engine bundle link:",
        err,
      );
      if (loadingDiv) loadingDiv.style.display = "none";
      isInternalLoading = false;
...
function getFingerDistance(state) {
  if (state.thumb && state.index)
    return Math.sqrt(
      (state.index.x - state.thumb.x) ** 2 +
        (state.index.y - state.thumb.y) ** 2,
    );
  return 1.0 - (state.pinchStrength || 0);
}

function applyDeadZone(delta, threshold) {
...
function smoothDampScalar(
  ...
  const change = current - target,
    temp = (velocityState[key] + omega * change) * deltaTime;
  velocityState[key] = (velocityState[key] - omega * temp) * exp;
  let output = target + (change + temp) * exp;
  if (target - current > 0 === output > target) {
    output = target;
    velocityState[key] = (output - target) / deltaTime;
  }
  return output;
}
...
function animate() {
  ...
  if (!isIdle && !state.closedFist && !state.openPalm) {
    ...
    let dX = applyDeadZone(state.x - rotationSession.startX, 0.012),
      dY = applyDeadZone(state.y - rotationSession.startY, 0.012);
    let tY = rotationSession.startRotY + dX * Math.PI * 1.16;
    let tX = rotationSession.startRotX - dY * Math.PI * 0.65;

    if (config) {
      if (config.minRotationX !== undefined)
        tX = Math.max(config.minRotationX, tX);
      if (config.maxRotationX !== undefined)
        tX = Math.min(config.maxRotationX, tX);
      if (config.minRotationY !== undefined)
        tY = Math.max(config.minRotationY, tY);
      if (config.maxRotationY !== undefined)
        tY = Math.min(config.maxRotationY, tY);
    }
    ...
    if (state.pinch) {
      ...
      targetTransform.scale = Math.max(
        baseScale * zoomMinForModel,
        Math.min(
          baseScale * zoomMaxForModel,
          zoomSession.startScale * zoomRatio,
        ),
      );
    } else {
    ...
    if (isIdle) {
      ...
      if (scaleDiff > 0.05 || pitchDiff > 0.05) {
        ...
        targetTransform.rotationX =
          Math.sin(elapsedTime * IDLE_PITCH_SPEED) * IDLE_PITCH_AMPLITUDE;
      }
    }
  }

  const activeSmoothTime = isIdle ? RECOVERY_SMOOTH_TIME : ROTATION_SMOOTH_TIME;
  currentTransform.rotationY = smoothDampScalar(
  ...
  currentTransform.rotationX = smoothDampScalar(
  ...
  currentTransform.scale = smoothDampScalar(
  ...
  if (monument) {
    ...
    monument.position.y =
      (monument.userData.floorY || 0) +
      Math.sin(elapsedTime * 1.6) * (0.08 / currentTransform.scale);
  }
